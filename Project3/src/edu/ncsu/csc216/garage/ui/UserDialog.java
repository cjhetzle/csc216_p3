...
public class UserDialog extends JDialog {
	 ...
	 * @param v vehicle for constructor
	 */
	public UserDialog(Vehicle v) {
		this();
		if (v != null) {
			String data = v.toString();
			//Set the vehicles type
			if (data.charAt(0) == 'E')
				ckbxKind.setSelected(true);
			//Set the tier of the vehicle
			cboTier.setSelectedIndex(v.getTier());
			//Set the customer name text field
			((JTextField)compInfo[0]).setText(v.getName());
			//Set the license name text field
			((JTextField)compInfo[1]).setText(v.getLicense());
		}
	}
